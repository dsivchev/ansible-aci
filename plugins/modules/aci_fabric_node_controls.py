...
options:
  dom_control:
    description:
    - Enable/Disable digital optical monitoring (DOM) monitoring.
    type: int
    default: disable
...
def main():
    ...
    argument_spec.update(
        dom_control=dict(type='str', choices=["enable", "disable"], default="disable", aliases=['control']), 
    ...
    if state == 'present':
        if dom_control == "enable":
            control = "Dom"
        elif dom_control == "disable":
            control = ""
        aci.payload(
            aci_class='fabricNodeControl',
            class_config=dict(
